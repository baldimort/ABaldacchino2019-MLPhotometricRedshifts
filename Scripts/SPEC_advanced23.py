...
glob.annz["outDirName"] = "SPEC_advanced23"
...
if glob.annz["doTrain"]:
  ...
  for nMLMnow in range(glob.annz["nMLMs"]): 
    ...
    # - example use:
    # --------------------------------------------------------------------------------------------------
    input_vars = "i_auto;umag;bmag;vmag;gmag;rmag;jmag;kmag;band0_flux;band2_flux;band4_flux;band6_flux;band8_flux;band10_flux;band12_flux;band14_flux;band16_flux;band18_flux;band20_flux;band22_flux;band24_flux;band26_flux;band28_flux;band30_flux;band32_flux;band34_flux;band36_flux;band38_flux"
    ...
    glob.annz["rndOptTypes"] = "ANN"
    glob.annz["userMLMopts"] = genRndOpts(nMLMnow)
    #if nMLMnow % 2 == 0:
    #    glob.annz["rndOptTypes"] = "BDT"
    #    glob.annz["userMLMopts"] = genRndOpts(nMLMnow)
    #else:
    #    glob.annz["rndOptTypes"] = "KNN"
    #    glob.annz["userMLMopts"] = genRndOpts(nMLMnow)
    # rndOptTypes - generate automatically these randomized MLM types for cases
    #               where [userMLMopts = ""] (currently "ANN", "BDT" or "ANN_BDT" are supported) - KNN added by Andy 08/03/19
    # --------------------------------------------------------------------------------------------------
...
if glob.annz["doOptim"] or glob.annz["doEval"]:
  ...
  # --------------------------------------------------------------------------------------------------
  glob.annz["max_sigma68_PDF"] = 0.055
